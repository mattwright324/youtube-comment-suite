...
public class CommentDatabase implements Closeable {
    ...
    YouTubeComment resultSetToComment(ResultSet rs) throws SQLException {
    ...
    public void insertGroupItems(Group g, List<GroupItem> items) throws SQLException {
        try(PreparedStatement psCG = sqlite.prepareStatement(SQLLoader.CREATE_GITEM.toString());
            ...
            for(GroupItem gi : items) {
                psCG.setString(1, gi.getYoutubeId());
                ...
                psCGG.setString(2, gi.getYoutubeId());
                psCGG.addBatch();
            }
    ...
    public long getTotalVideos(Group group) throws SQLException {
        try(PreparedStatement ps = sqlite.prepareStatement(SQLLoader.GET_GROUP_TOTAL_VIDEOS.toString())) {
            ps.setString(1, group.getId());
            try(ResultSet rs = ps.executeQuery()) {
                if(rs.next()) {
                    return rs.getLong("count");
    ...
    public long getTotalViews(Group group) throws SQLException {
        try(PreparedStatement ps = sqlite.prepareStatement(SQLLoader.GET_GROUP_TOTAL_VIEWS.toString())){
            ps.setString(1, group.getId());
            try(ResultSet rs = ps.executeQuery()) {
                if(rs.next()) {
                    return rs.getLong("total_views");
                }
            }
        }
        return 0;
